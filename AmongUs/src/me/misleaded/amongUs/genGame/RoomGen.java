...
public class RoomGen {
	...
	private int[] room;
	private RoomGen[] children = new RoomGen[2];
	private int[][] rooms;
...
Neg Z	----------+---------- Pos X
	...
	private int[][] getCorridor(int[] r1, int[] r2) {
...
//		p3 = (randRange(0, 2) == 0) ? new int[] { p2[0], p1[1] } : new int[] { p1[0], p2[1] };
	...
	private HashMap<String, Object> genRoomsCorridors() {
		if (depth == 0) {
...
//		       1
	...
	public static void setBlockInNativeChunk(World world, int x, int y, int z, Block b, boolean applyPhysics) {
	...
	public void drawMap(World world) {
		Block[] layout = { Blocks.QUARTZ_BLOCK, 
							  Blocks.STONE_BRICKS, 
							  Blocks.SEA_LANTERN, 
							  Blocks.STONE_BRICKS, 
							  Blocks.GLASS, 
		...
		for (int y = 0; y < layout.length; y++) {
			for (int w = -1; w < width; w++) {
				for (int h = -1; h < height; h++) {
					setBlockInNativeChunk(world, topLeft[0] + w, 199 + y, topLeft[1] + h, layout[y], false);
			...
			for (int y = 0; y < 4; y++) {
				for (int w = 0; w < room[0]; w++) {
					for (int h = 0; h < room[1]; h++) {
						setBlockInNativeChunk(world, room[2] + w, 200 + y, room[3] + h, Blocks.AIR, false);
				...
				for (int y = 0; y < 4; y++) {
					for (int w = 0; w < segment[0]; w++) {
						for (int h = 0; h < segment[1]; h++) {
							setBlockInNativeChunk(world, segment[2] + w, 200 + y, segment[3] + h, Blocks.AIR, false);
